...
**Glossary (first use):** POA = Plan of Action (Amazon's required appeal document). MV3 = Manifest V3, Chrome's current extension platform. CWS = Chrome Web Store. MoR = Merchant of Record (payment provider that legally resells your product and handles VAT/sales tax — here Paddle primary, Polar fallback). BSA = Amazon's Business Solutions Agreement; its §19 "Agent Policy" (effective 4 Mar 2026) restricts automated tools on Seller Central. SW = extension service worker. Nano = Gemini Nano, Chrome's built-in on-device model (`LanguageModel` API). EU-DSA = EU Digital Services Act (requires trader verification on the CWS listing).
...
| **Web revenue surface** | 4–5 | Free notice decoder + paid $199 composer live on the AppealDeck domain. Cloud Gemini Flash **paid tier** does the drafting; MoR (Paddle, Polar fallback) checkout; license keys self-issued via Supabase. | First revenue with no store gate; catches mobile panic-searchers; revenue bridge if CWS review stretches weeks. [source: The Second Opinion.md] |
...
3. **DOM-harvest code is not merged until Jhangir has retrieved and the team has read the full BSA §19 / Agent Policy text** (it sits behind seller login). If the read is bad, the extension de-scopes to paste-only — the product still works completely.
...
Owners: **AI assistant** (the coding agent executing the v1.0 spec + amendments), **Founder** (accounts, approvals, money, community), **Jhangir** (Seller Central access under written consent, §19 text, PK talent network — see `../08-TEAM/` for the partnership agreement prerequisites). "Week 1" is the first week of Phase 2, entered only after the Phase-0 blockers and Gate 1 checks pass (see `../01-PHASE-0-BLOCKERS/` and `../02-PHASE-1-FOUNDATION/`).

### Week 1 — prerequisites + core start (M-1, M-2 begins)

- [ ] **B-01** Confirm Phase-0 blockers closed: leaked Postgres/Supabase credentials rotated, Jhangir written agreement signed BEFORE any credential sharing. — **Owner:** Founder · **Cost:** $0 (rotation) · **Deadline:** Day 1 · **Blocks:** everything below.
...
- [ ] **B-04** Open CWS developer account ($5) and start EU-DSA trader verification (public contact details on the listing) — verification can take weeks, so it starts now, not at submission. — **Owner:** Founder · **Cost:** $5 · **Deadline:** Week 1 · **Blocks:** M-7.
...
- [ ] **B-06** Apply to BOTH Paddle and Polar behind the live site + legal pages (D2). Paddle primary if approved; Polar Pro fallback. — **Owner:** Founder · **Cost:** $0 (Polar Pro $20/mo when activated) · **Deadline:** Week 1, approval needed by Week 5 · **Blocks:** M-W checkout.
...
- [ ] **B-08** Request the full BSA §19 / Agent Policy text (behind seller login) via Jhangir's Seller Central access; deliver as PDF/text into `../07-REFERENCE/`. — **Owner:** Jhangir · **Cost:** $0 · **Deadline:** before M-3 (end of Week 2) · **Blocks:** any DOM-harvest code merge, injector decision, M-6 scope.
...
- [ ] **B-13** Run 5–8 paid auditions at $15–50 each (~$75–400 total) per `../08-TEAM/02-RECRUITMENT-KIT.md` §2 (fixture notice with the two traps: ambiguous window + missing invoice; fabricating the invoice = instant fail). Every contact founder-approved; Fiverr/Upwork orders stay on-platform. — **Owner:** Founder + Jhangir (PK network) · **Cost:** ~$75–400 · **Deadline:** Week 2 · **Blocks:** template quality review, M-4.
...
- [ ] **B-17** Install EU-hosted analytics (Plausible or Umami) on the web decoder; define the D10 funnel events: decoder session → decode completed → intake started → purchase → opt-in outcome. — **Owner:** Founder (account), AI assistant (integration) · **Cost:** €0–50/mo · **Deadline:** Week 3 · **Blocks:** weekly funnel review, price experiment measurement.
...
- [ ] **B-25** Ask design partners for live Seller Central QA access (the AM-04 ladder: Jhangir first with written read-only consent, design partners asked Week 4 for access by Week 6, fresh Individual registration ~1–2 wks as last resort). Recruit 5 design partners with live violations to bank on 3 completions (free Pass barter; per `../04-PHASE-3-LAUNCH/03-DESIGN-PARTNER-BETA.md`). — **Owner:** Founder + Jhangir · **Cost:** $0–40 · **Deadline:** ask Week 4, access by Week 6 · **Blocks:** M-6 gate only (fixtures cover everything earlier).
- [ ] **B-26** M-6 polish: popup (M10), options routes complete (M9), M14 copy everywhere, remote-settings kill switch tested, staged-rollout plan written (AM-14 — staged rollout applies only once the install base exceeds CWS partial-rollout eligibility, ~10,000 users; below the threshold every release is effectively big-bang, substituted by hardened pre-release QA + the tested kill switch). Injector: build behind flag if and only if B-15 ruled it in; flag ships OFF. Pass the M-6 gate: "Manual QA clean on live Seller Central (design-partner account); 'guarantee' grep = 0". — **Owner:** AI assistant (build), Founder + Jhangir (live QA) · **Cost:** $0 · **Deadline:** Week 6–7 · **Blocks:** M-7.
...
- [ ] **B-28** Design-partner beta on the unlisted build: 3 real cases decoded end to end; collect testimonials and (opt-in) outcome data. Support channels + canned responses live before the beta opens. — **Owner:** Founder (+ Jhangir community triage) · **Cost:** $0 · **Deadline:** Week 7–8 · **Blocks:** M-7 gate, listing copy.
