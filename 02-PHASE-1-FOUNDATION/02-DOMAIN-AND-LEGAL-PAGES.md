...
Glossary: **POA** = Plan of Action (the appeal document Amazon requires from a suspended seller). **MoR** = Merchant of Record. **GDPR** = the EU General Data Protection Regulation. **DPA** = Data Processing Agreement (the GDPR Article 28 contract you sign with each vendor that processes personal data for you). **ROPA** = Record of Processing Activities (GDPR Article 30). **DSR** = data-subject request (access/export/delete). **KKV** = the Finnish Competition and Consumer Authority. **tietosuoja.fi** = the Finnish Data Protection Ombudsman (the supervisory authority for breach notifications).
...
- Optionally also register the `.com` if cheaply available, redirecting to the primary — protects the brand for a few euros. Optional, not blocking.
...
| **About / Contact** | Who runs it: the founder's real name, business name (Hawlton Alliance, Finland), contact email on the domain. Matches the identity on the Paddle application and the CWS trader verification exactly — identity mismatches are a documented Paddle rejection trigger (unverified — third-party rejection analyses). |
...
2. **License validation.** A random device identifier and license key are sent to our backend (Supabase, EU region) to check entitlement. No case content is included.
...
Also disclose: the analytics tool (Plausible or Umami — cookieless, aggregate only), and that AI assistance is used to generate draft documents (EU AI Act Article 50 transparency — AI-generated content must be identifiable to the user).

### 3.2 GDPR duties (founder obligations behind the policy)

| Duty | What to do | Tooling/cost |
|------|-----------|--------------|
| **Article 30 ROPA** | Maintain a record of processing activities: license data, cloud-LLM payloads, telemetry, support email. A spreadsheet is legally sufficient at this scale. | tietosuojatyokalu.fi — verify current pricing (conflicting reports: trial-then-paid); free template alternative otherwise |
| **DPAs with subprocessors** | Execute/accept the standard DPA of each processor: **Paddle and/or Polar** (payments), **Supabase** (backend), **Cloudflare and/or Vercel** (hosting), **Google** (Gemini API). All offer standard self-serve DPAs with EU Standard Contractual Clauses. Keep signed/accepted copies on file. Publish the subprocessor list on the site. | $0 |
| **72-hour breach notification** | If personal data is breached, notify the Finnish Data Protection Ombudsman via **tietosuoja.fi** within 72 hours of awareness, and affected users when the risk is high. Keep a one-page internal SOP: detect → contain → assess scope → notify → document. Links into `../06-OPERATIONS/` (crisis playbook). | $0 |
...
7. **Governing law:** Finland, without prejudice to mandatory consumer protections of the customer's home country.

### 4.2 Refund policy (decision D8 — binding)

- **7-day voluntary refund, no questions asked**, for the Appeal Pass. Fast refunds beat disputes: chargeback thresholds are existential for a MoR relationship (Polar reviews at 0.4%; card networks ~0.75–1.5%), and a MoR termination kills the business. Never argue with a refund request inside the window.
...
- [ ] **9.** Ensure the checkout consent checkbox + durable-medium confirmation are configured on the chosen MoR (verify Paddle/Polar checkout supports custom consent text; if not, place the consent step on our own pre-checkout page). — **Owner:** AI assistant (Founder verifies) · **Cost:** $0 · **Deadline:** before checkout goes live · **Blocks:** enforceable delivery-before-withdrawal
...
- Scoped external legal review (ToS + privacy + withdrawal mechanics, EU/Finnish angle): ~€500–1,500 — recommended before any paid US marketing spend, not before launch.
