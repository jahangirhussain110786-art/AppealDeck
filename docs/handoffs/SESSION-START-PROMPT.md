...
**Current pass (11 Sep 2026, later the same day):** the founder approved a new visual direction (mockups inspired by Stripe Atlas / Ironclad / 1Password / Linear) and asked to overhaul the entire real application to match it, "professionally and perfectly." This is locked in at `docs/handoffs/2026-09-11-visual-overhaul-v4-handoff.md` — read it in full first; it answers the token-architecture question, cites the exact approved mockup source (copied into `docs/handoffs/2026-09-11-visual-direction-v2-mockup/`), and lays out a token-first task order. **No dedicated task-by-task prompt/spec exists yet** — same situation as the AA-31 feature work below: this repointing names a source-of-truth document, not a written prompt. **A fresh session's first job is to write that prompt** (an `AM-2x` amendment: STATUS banner, §0 rules, Appendix A anchors, one task per commit with an Accept block, matching every prior pass's structure) per the handoff's §3 task order, then run it. The prior "what's next" item — the remaining AA-31 M-4 feature work (ASIN/case-ID extraction, seller override of decoded type, root-cause category step, dated corrective-action UI, character counter; full list in `02-BUILD-PLAN-AMENDMENTS.md`) — is still open but the founder's explicit instruction puts the visual overhaul first; do not start AA-31 instead without checking with the founder.
...
PASS PROMPT PATH: docs/handoffs/2026-09-11-visual-overhaul-v4-handoff.md
EVIDENCE LOG PATH: docs/handoffs/2026-09-11-visual-overhaul-v4-handoff.md
