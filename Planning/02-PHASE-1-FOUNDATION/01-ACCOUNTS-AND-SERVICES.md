...
**Why this file exists / when to use it:** Every external account AppealDeck depends on is listed here with its cost, owner, opening order, and what it blocks downstream. Work through it in Week 1–2, in the order given — several items (Chrome Web Store trader verification, payout account verification, Paddle review) have multi-day lead times, so opening them late silently delays later milestones. Prerequisites from Phase 0 (leaked-credential rotation) must be done first — see `../01-PHASE-0-BLOCKERS/`.

Glossary for this file: **CWS** = Chrome Web Store. **MoR** = Merchant of Record (a payment provider that legally resells your product and handles VAT/sales tax). **LLM** = large language model.
...
|---|---------|---------|-------|------|------|--------|
| 1 | Chrome Web Store developer account + trader contact details | Publish the extension | Founder | $5 one-time | Week 1, Day 1–2 | Store submission (Week 7–8); listing visibility |
| 2 | Domain + business email | Site, legal pages, public contact address | Founder | ~$10–30/yr | Week 1, Day 1–3 | Legal pages, Paddle application, CWS privacy-policy URL, contact email |
...
| 4 | Hosting: Cloudflare Pages (default) — Vercel Pro is an M-5 fallback only, not a co-equal option | Host the landing page, legal pages, and the free web decoder | Founder opens; AI assistant deploys | $0 (default path) | Week 1–2 | Live site → Paddle application; web decoder (Week 4–5 revenue surface)  **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`. |
...
| 6 | Wise (personal or business) | Cross-border payouts and Pakistan contractor payments (PayPal is unavailable in Pakistan) | Founder | No monthly fee; 0.35–1% per transfer | Week 1–2 (verification takes days) | Paid expert auditions (Week 2), PK retainers |
| 7 | Analytics: Plausible **or** Umami | Decoder funnel metrics (decision D10) | Founder picks; AI assistant wires events | Plausible from ~€9/mo (estimate — verify current price); Umami self-hosted $0 | Week 2 | Funnel measurement from the first decoder session |
...
| 9 | Polar account | Warm-fallback MoR — verify the Pakistan payout path (Stripe Connect cross-border) at signup; Dodo Payments (MoR, 4% + 40¢) is plan-C: application-ready, no account opened until triggered | Founder | $0 (free tier) | Week 1 — details in `./03-PAYMENTS-SETUP.md` | Payment fallback path |
...
Not in this file: an **active Amazon Seller Central account** for live-page QA. It blocks only the Week 6–7 live-QA gate, is sourced through the founder's own fresh Amazon Individual seller account (free, ~1–2 weeks) or a design partner's read-only secondary-user invite with written consent, and is handled in the build and team phases (see `../03-PHASE-2-BUILD/` and `../08-TEAM/`).

---

## 2. Per-account steps

### 2.1 Chrome Web Store developer account (+ trader contact details)

The $5 registration is trivial; the reason this is a Day-1 item is that CWS requires verified contact details (name/business name, address, email, phone) on the listing. Verification is a back-and-forth with lead time — start it early.

- [ ] **1.** Register a CWS developer account under the seller identity (Jhangir Hussain / Hawlton brand), using the business email from item 2 — not a personal Gmail. — **Owner:** Founder · **Cost:** $5 one-time · **Deadline:** Week 1, Day 1–2 · **Blocks:** store submission (Week 7–8)
- [ ] **2.** In the developer dashboard, complete the trader declaration and start contact-details verification immediately (submit name/business name, address, email, phone for verification). — **Owner:** Founder · **Cost:** $0 · **Deadline:** started Week 1 · **Blocks:** listing visibility at launch
...
- [ ] **6.** Create a new Supabase project in a region of your choice under a fresh organization. Do NOT reuse or link the compromised project. — **Owner:** Founder · **Cost:** $0 (free tier) · **Deadline:** Week 1 · **Blocks:** licenses table, payment webhooks, LLM proxy
...
- [ ] **10.** Create a Cloudflare account; connect the domain (DNS + Pages). — **Owner:** Founder · **Cost:** $0 · **Deadline:** Week 1–2 · **Blocks:** live site → Paddle application **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`.
- [ ] **11.** Deploy a placeholder→real landing site via Cloudflare Pages per `./02-DOMAIN-AND-LEGAL-PAGES.md`. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** Week 1–2 · **Blocks:** Paddle application (needs a real, navigable site) **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`.
...
### 2.6 Wise (personal or business)

Cross-border payouts and Pakistan contractor payments. PayPal does not operate in Pakistan; Wise personal or business works for both receiving MoR payouts and sending PK payments at 0.35–1% per transfer with no monthly fee. Verification takes days — open it before you need it.

- [ ] **15.** Open a Wise account in the founder's own name (or business name if incorporating later); complete verification. — **Owner:** Founder · **Cost:** $0 setup; 0.35–1% per transfer · **Deadline:** Week 1–2 · **Blocks:** MoR payout receipt, paid fixture auditions (Week 2), PK contractor payments
- [ ] **16.** Keep every transfer receipt for record-keeping. — **Owner:** Founder · **Cost:** $0 · **Deadline:** ongoing · **Blocks:** clean records

### 2.7 Analytics — Plausible or Umami

Decision D10 sets the north-star metric (paid Appeal Passes/week) and the funnel: decoder sessions → decode completed → intake started → purchase → opt-in outcome. Both Plausible and Umami are cookieless, privacy-friendly, and require no consent banner for anonymous stats — still disclosed in the privacy policy.

- [ ] **17.** Pick one: Plausible (hosted, from ~€9/mo — estimate, verify) for zero maintenance, or Umami (self-hosted, $0) if the founder accepts running it. Either satisfies D10. — **Owner:** Founder · **Cost:** $0 (Umami) or ~€9/mo (Plausible) · **Deadline:** Week 2 · **Blocks:** funnel measurement from first decoder session
...
2. Founder retains sole ownership of all primary accounts (decision D5). Any future collaborator gets a scoped invitation, never shared credentials — and nothing is shared with anyone the collaborator policy does not bind (see `../01-PHASE-0-BLOCKERS/02-COLLABORATOR-POLICY.md`).
...
- [ ] CWS developer account registered ($5 paid) AND trader contact details submitted.
- [ ] Domain registered, business email working, DNS on Cloudflare.
- [ ] Fresh Supabase project exists; old compromised project untouched/decommissioned; nightly dump job running.
- [ ] Hosting default confirmed (Cloudflare Pages + Supabase Edge Functions; Vercel Pro remains an M-5 fallback only); site deploys. **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`.
- [ ] Gemini DEV key created; PROD (paid, billing-enabled) key plan documented; both keys backend-only.
- [ ] Wise verified and able to send a test transfer.
- [ ] Analytics tool chosen and account created.
- [ ] Paddle application underway per `./03-PAYMENTS-SETUP.md`; repo + CI live per `./04-REPO-AND-FIXTURE-CORPUS.md`.
