...
| Live Seller Central QA session on a partner account | The M-6 acceptance gate: manual QA clean on a **live** account. Fixtures cover milestones M-1→M-5; only live access proves the panel mounts, harvest works, and no console errors appear on real pages. Access ladder: design partners first (written read-only consent via a secondary-user invite — the §6 one-pager is the consent record), the founder's own fresh Individual-plan registration last (shows a clean page only) | `../00-DECISION/03-GATES-AND-KILL-CRITERIA.md` check 15 [source: The Second Opinion.md, amendment 4] |
...
1. **Who we are:** the founder's name — Jhangir Hussain (individual operator, Pakistan) — contact email, and the product's one-line description.
...
- [ ] **8.** Run the M-6 supervised live QA session on the partner account (or, as fallback, the founder's own fresh Individual account); record results per the QA checklist. — **Owner:** Founder (+ partner) · **Cost:** $0 · **Deadline:** Week 6–7 · **Blocks:** M-6 acceptance, Gate 2
