...
- [x] **AA-01** Apply the §2.2/§2.5/§9.2/M11 payment substitutions above throughout the codebase and docs; grep `lemonsqueezy|LEMONSQUEEZY|ls_` → 0 hits. **Done, verified 11 Sep 2026:** `src/app/api/webhooks/paddle/route.ts` implements the full Paddle path; `grep -ril lemonsqueezy src/` → 0 hits. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** Week 1 (spec), Week 4–5 (code) · **Blocks:** M-W, M-5.
...
- [x] **AA-05** Implement `src/core/deadlinesModel.ts` to this table; unit tests cover all six kinds + the fraud no-countdown state. **Done, verified 11 Sep 2026:** all six deadline kinds implemented (`appeal_window`, `funds_appeal_eligible`, `funds_review`, `seller_challenge`, `aha_72h`, `indefinite_hold`), exercised by passing tests. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** with M-4 · **Blocks:** M-4 gate.
...
- [x] **AA-08** Implement checkout consent flow (explicit prior consent + permanent-form confirmation) and the 7-day voluntary refund policy in terms + refund handling SOP; delete every "no refund after generation" phrase from spec-derived copy. **Done 11 Sep 2026:** `src/components/pricing/ConsentRow.tsx` + `src/content/legal.ts` implement the consent checkbox (purchase disabled until checked) and refund copy; the permanent-form confirmation email is now sent by `src/lib/email.ts`/the Paddle webhook (see the 11 Sep audit-guidebook Section F3 fix). — **Owner:** AI assistant (implementation), Founder (policy sign-off) · **Cost:** $0 · **Deadline:** before M-W goes live · **Blocks:** M-W, MoR compliance.
...
- [x] **AA-10** Rewrite §11 headings/emphasis in the working spec copy (`docs/BUILD_PLAN.md`): "11.1 Cloud (primary quality path)", "11.2 On-device Nano (opportunistic)"; route composer/critic to cloud by default for paid cases; route triage/classification to Nano when `availability() === 'available'`. **Engineering done, doc deliverable descoped (11 Sep 2026):** the actual routing (`src/lib/llm/gemini.ts` — per-task model matrix, per-task env overrides, `thinkingConfig.thinkingBudget: 0`, `withGeminiBreaker()` wrapping) is implemented and correct; `docs/BUILD_PLAN.md` was never created and won't be — this file plus `docs/DECISIONS.md` already carry the same information, and a second working-spec copy would just be one more place for drift. — **Owner:** AI assistant · **Cost:** ~$0.02/case cloud (estimate) · **Deadline:** with M-4/M-W · **Blocks:** M-W quality bar.
...
- [ ] **AA-12** Provision the settled launch stack: Cloudflare Pages (free) for the decoder + Supabase Edge Functions for all API work; upgrade Supabase to Pro $25/mo at first sustained sales. — **Owner:** Founder · **Cost:** $0–25/mo · **Deadline:** Week 2 · **Blocks:** M-W. **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`.
...
- [x] **AA-14** Implement activation limit + self-service deactivation; acceptance: 6th device activation is refused with a clear UI path to deactivate an old device. **Done, verified 11 Sep 2026:** `src/lib/devices.ts` implements a 5-device cap with `over_cap` status; `src/lib/__tests__/devices.test.ts` passes ("5th device is the last accepted; 6th is over_cap with a list"). — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** before M-5 gate · **Blocks:** Gate 3 ("anti-piracy controls active", see `../04-PHASE-3-LAUNCH/`).
...
- [x] **AA-15** Implement cap + limits + degradation; acceptance: simulated spike trips the breaker and both clients degrade to rules-only visibly and recover automatically next budget window. **Done, verified 11 Sep 2026:** `src/lib/breaker.ts` implements spend cap, per-minute rate limiting, and circuit-open states with a typed `DegradedResponse`. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** before M-W public · **Blocks:** M-W gate.
...
- [x] **AA-16** Write `docs/MIGRATIONS.md` (strategy + checklist) and the 10k-record migration test harness; both exist before the FIRST schema change after v1 ships. **Done, verified 11 Sep 2026:** `docs/MIGRATIONS.md` exists; `src/core/vault/migration.test.ts` runs a 10,000-record v1→v2 migration harness, passing with zero loss. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** with M-6 · **Blocks:** any post-launch schema change.
...
- [x] **AA-21** Wire the EF-5 outcome schema into the D10 analytics events; add the quarterly policy re-check to the ops cadence; founder adds the two new banned rows ("agency results at 1/10th the cost"; readiness-as-approval framing) to `../07-REFERENCE/01-MARKET-EVIDENCE.md` §4. **Done, all three parts, 11 Sep 2026:** the EF-5 schema is wired end to end (`src/core/outcomeModel.ts`, `POST /api/outcome`, `supabase/migrations/0008_outcome_events.sql`, `OutcomeShareCard.tsx`) and reports through the D10 analytics events (`trackFunnelEvent(FUNNEL_EVENTS.outcomeShared, ...)` in `src/lib/analytics.ts`); the quarterly policy re-check is item 5 of `01-MARKET-EVIDENCE.md`'s action list, joining item 3's existing cadence; the two banned rows are added to `01-MARKET-EVIDENCE.md` §4. **Not yet live:** the migration needs a manual apply in the Supabase dashboard (`docs/DEPLOYMENT.md` §4) before any real outcome row can be written. — **Owner:** AI assistant (schema/cadence), Founder (banned rows) · **Cost:** $0 · **Deadline:** with M-W · **Blocks:** any future outcome statement; F4 staleness control.
...
- [x] **AA-22** Implement `caseState.ts`, objection-extended actions, `responseAnalyzer.ts` + reply fixtures, with unit tests. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** with M-4 (B-18) · **Blocks:** M-4 gate (amended above). — **DONE 2 Sep 2026:** `src/core/caseState.ts` (DECODED → INTAKE → REMEDIATION → READY → SUBMITTED → AWAITING → APPROVED/REJECTED/NO_RESPONSE, with REVISION requiring novelty, FUNDS_TRACK lock until +60d, GATED_PRO_HELP short-circuit), `src/core/responseAnalyzer.ts` (categorical analyzer with ≥2 fixtures per category + adversarial case), `ActionItem.declined` + `alternatives[]` enforced by the composer (fabrication-flavored alternatives refused). Objection alternatives per kind are SKELETON-only pending consultant review (AA-25/AA-20). 133 tests green. — **Corrected 24 Sep 2026:** the objection alternatives were unreachable from 22 Sep (`d9cb847`) until 23 Sep, when `alternativesFor()` was rebuilt on the workspace as the "cannot obtain" path (`0bbff36`, gap A-02). The tick is true again, of the workspace version.
...
- [x] **AA-28** Wave C — app confidence: dashboard = caseState home, interview Stepper + resume + encrypted save-and-exit + mobile action bar, compose PoaSection + critic margin + BeforeYouSubmitChecklist (hosts PoaEditor), vault teaching state + icons + search/filter, billing/device polish, AuthCard; plus founder ratifies AM-18 and signs off Waves A+B screenshots. **Done, verified 11 Sep 2026:** `PoaSection.tsx`, `BeforeYouSubmitChecklist.tsx`, `Stepper.tsx`, `AuthCard.tsx`, `VaultLockedState.tsx` all exist and are wired into `ComposeView.tsx`/`InterviewFlow.tsx`/the auth pages; AM-18 ratified 9 Sep. — **Owner:** AI assistant (build), Founder (ratify + sign-off) · **Cost:** $0 · **Deadline:** with M-W closure · **Blocks:** M-W gate.
- [ ] **AA-29** Copy audit + content modules: inventory every user-facing string in `src/`, move to `src/content/{marketing,app,auth,legal,errors}.ts`, rewrite per spec §10.1–10.3 (before/after table in the PR), wire the §10.5 banned-pattern lint into CI with an allow-list, fix the two known hits (privacy page "privacy-first" ×2, checkout "secure checkout"). — **Owner:** AI assistant (audit + rewrite), consultant (B-16 review of expectations/triage copy) · **Cost:** $0 · **Deadline:** with Wave B, before first deploy · **Blocks:** M-W gate (copy lint added to the amended gate). — **5 Sep 2026 status:** marketing + legal copy complete (committed `e966c19`); app + auth strings still inline in pages/components — move to `content/app.ts` / `content/auth.ts` and wire in Wave C. **Done, verified 11 Sep 2026:** `src/content/app.ts` and `src/content/auth.ts` exist and are wired throughout the app and auth pages; the banned-pattern sweep (`seamless`, `privacy-first`, `trust us`, `secure checkout`, `AI-powered`, `hassle`) returns 0 hits.
...
- [ ] **AA-31 — PARTIAL, 11 Sep 2026.** **Done:** the deterministic critic rules themselves — future-tense corrective actions, blame-shifting words, vague-time phrases, invoice issue-date freshness flag (`documentDate` field + `freshnessDays` check), attachment-by-filename nudge — all implemented in `src/core/composer.ts` as warning/info findings, never errors (`src/core/composer.test.ts`, 10 new tests including one asserting none of the five new rules ever produce an error-severity finding). The ID paste normaliser was built (`src/lib/idNormalize.ts` + tests) but has no input to serve — the product has no ID field (gap A-08, 23 Sep 2026); only its invisible-character stripping is used, on stored notice text. **Not done, genuinely separate scope:** ASIN + case-ID _extraction from the notice text_ (a parser/classifier feature, not a critic rule — belongs in `src/core/index.ts`'s decode pipeline); seller override of the decoded type (a UI affordance on the decode result); a root-cause category step (an interview-flow addition); dated corrective-action rows and a neutral character counter (both UI, on `PoaSection.tsx`/the action-item editor); "jargon swaps" (never scoped precisely enough to implement — needs a concrete banned-jargon list from the founder or a consultant before it can be built). — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** with M-4 · **Blocks:** M-4 gate (partially cleared).

### AM-20 — Visual system v3: brand mark v2, tokens v3, tinted semantics, hero-as-product, shell rebuild (ratified 10 Sep 2026)
...
- [x] **AA-32** Visual refresh v3 per `docs/handoffs/2026-09-09-visual-refresh-prompt.md` Tasks V0–V11, evidence in `docs/handoffs/2026-09-09-visual-refresh.md`. **Done 11 Sep 2026** (`b593398` V1 · `593908e` V2+V3 · `13c3b9f` badge fix · `6fa862d` V4–V6 · `6e041a1` V7–V9 · `d720bd2` V10+V11 code). §5 gates at `d720bd2`: tsc 0 · lint 0 · lint:copy PASS · format 0 · vitest 332/332 · build 32 routes · Playwright 96/0/30 skipped · Lighthouse a11y 1.0 on all five pages. Four defects found by the pass's own gates and fixed (decode result crash on `getTime`, signed-out `/vault` `h1`, footer heading order, dark-mode `VerifiedStamp`). Open for the founder: hero headline renders on 4 lines at 1280 (spec value vs. copy-deck text — evidence log Deviations); screenshot sign-off; auth-gated captures NOT RUN (no credentials). — **Owner:** AI assistant (build), Founder (mark + sample-notice approval, screenshot sign-off) · **Cost:** $0 · **Deadline:** before first deploy · **Blocks:** M-W gate (rides the AM-18 gate).
...
### AM-22 — Visual overhaul v4: token reconciliation, primitives, and page-by-page rebuild against the approved mockup (ratified 11 Sep 2026)

**Provenance:** founder direction in chat with the reviewing AI, 11 Sep 2026, after several design-canvas mockup iterations fixing copy accuracy, replacing stock photography with original illustrations, correcting font choice, and fixing cross-artboard consistency: _"everything else looks great... I would like to overhaul our entire application in all means to look even strong, perfect, well designed masterpiece."_ Design authority: `docs/handoffs/2026-09-11-visual-overhaul-v4-handoff.md`. Approved mockup source copied into the repo at `docs/handoffs/2026-09-11-visual-direction-v2-mockup/*.dc.html` (Main/Interview/Decode/Vault + `canvas.json`). Runs after the founder-issues fix pass (composer, password toggle, vault preview/download — complete at `1e4dd05`).

**Scope guard:** presentation, assets, copy, and layout only, per the AM-18 scope guard carried over explicitly — no new API routes, no engine or capability changes, D1–D10 not reopened.

**What changes:** token-first reconciliation of the mockup's warm-neutral palette and radius scale against the real `globals.css`/`tailwind.config.ts` (a founder-gated color call — adopt warm-neutral or keep the current cooler-neutral tokens with only the brand-green hue matched), a new Newsreader italic accent font for headline emphasis words; a Badge `size="sm"` variant, an aurora/dot-grid hero background utility, and a small layered-SVG illustrations library (shield-check, vault-door, magnifier-over-document); page rebuilds for home, the guided interview (step-rail + "why we ask" context panel), the decode result (two-column annotation cards), and the vault surface (a second founder-gated call — permanent dark "obsidian" register vs respecting the site-wide theme); a cross-page consistency sweep; and a final gate + screenshot set for founder sign-off.

- [ ] **AA-35** Visual overhaul v4 per `docs/handoffs/2026-09-11-visual-overhaul-v4-prompt.md` Tasks V0–V8, evidence in `docs/handoffs/2026-09-11-visual-overhaul-v4.md`. **V0 (paperwork) DONE 11 Sep 2026** (`533dc84`): formal task prompt, evidence log, this AM-22 entry, `docs/DECISIONS.md` entry, `SESSION-START-PROMPT.md` repointed; all gates re-verified green (tsc 0 · lint 0 · lint:copy PASS · format:check 0 · vitest 373/373 · build 33 routes — corrected from an earlier "32 routes" miscount carried across prior evidence logs, not a regression). **Corrected 24 Sep 2026:** V1–V6 and V8 shipped on 12 Sep (`39a09a8`, `4bf76b0`); V7, the cross-page sweep, is the one task never run. The two founder confirmations were given on 12 Sep. — **Owner:** AI assistant (build), Founder (the two founder-gated token/theme decisions; screenshot sign-off at V8). · **Cost:** $0 · **Deadline:** no hard deadline — presentation pass, does not block M-4 or first deploy. · **Blocks:** nothing downstream; runs independently of the composer/engine work.

### AM-23 — Real AI-drafted composer: an LLM rewrite layer over the deterministic draft (founder-authorized 12 Sep 2026, backfilled 21 Sep 2026)

**Provenance:** this entry did not exist until now. Code comments across `src/app/api/compose/route.ts`, `src/lib/llm/composePoaLlm.ts`, and `src/lib/llm/gemini.ts` had cited "AM-23" since 12 Sep 2026, but nobody ever wrote the amendment itself — the exact gap the 19 Sep Case OS v2 direction doc (§6 item 6) flagged and the founder's own paused second-opinion research never got back to. Recovered here verbatim from the code's own commit-time comment, since no `docs/DECISIONS.md` entry or handoff exists for this day's work: founder direction in chat, 12 Sep 2026 — _"whatever it is fix this any way and make sure there we get a real POA, not just text."_ This was the founder gate the composer-fix prompt (11 Sep) had explicitly left closed ("Task 4 — real AI-drafted composer — is explicitly founder-gated, do not start without the founder saying so in chat"); this is that go-ahead.

**What changed:** `composePoa()` (`src/core/composer.ts`) was left untouched as the deterministic, always-available fallback that prints the seller's own words verbatim. A new layer, `composePoaLlm.ts`, calls Gemini to rewrite only the Root Cause and Preventive Measures sections into professional appeal prose, grounded only in facts the seller actually supplied — never Corrective Actions (built from checkable, attested action items — D6, never invented) and never the Evidence Gaps section (deterministic). Falls back to the deterministic draft when Gemini is unconfigured or the narrative is insufficient. Uses the existing per-task Gemini breaker (`draft-poa-section`), so D9's cost ceiling and circuit breaker already cover it — no new infrastructure risk.

**Consequence for the "AppealDeck has no decision engine" line in older docs:** partly retired by this amendment alone, independent of the 21 Sep Phase-1 finding that made the same correction from the outside. `docs/handoffs/2026-09-11-founder-issues-diagnosis-and-plan.md`'s "there is no LLM call anywhere in `/api/compose`" is now historical, not current.

- [x] **AA-36** Real AI-drafted composer per the founder's 12 Sep go-ahead. **DONE 12 Sep 2026** (exact commit not recoverable from this session — no `docs/DECISIONS.md` entry exists for this day's LLM-composer work; the code itself is the only surviving record). — **Owner:** AI assistant (build), Founder (the go-ahead itself, already given). · **Cost:** $0 (Gemini paid tier, D9 ceiling applies). · **Deadline:** met. · **Blocks:** nothing — this is a backfill of already-shipped, already-running work.

### AM-24 — Interview asks "done, will do, or can't" before requesting a file (founder direction, 12 Sep 2026, backfilled 21 Sep 2026)

**Provenance:** same gap as AM-23 — cited in code (`src/core/interviewEngine.ts`, `src/core/readiness.ts`, `src/lib/caseStore.ts`) since 12 Sep 2026, never written up. Recovered from the code's own comment: founder direction, 12 Sep 2026 — sellers should be asked whether a corrective action is already done, still planned, or not possible, rather than being sent straight to a file-upload box for something that may not exist yet.

**What changed:** each readiness action item gained an `actionCheckAnswer: "done" | "will_do"` field. `"done"` means the interview now asks for the proof (the file step); `"will_do"` means the seller intends to but has not yet acted, so the file step is skipped for now and the item's status moves to `"in_progress"` instead of demanding a document that cannot honestly exist yet. A one-time banner explains this at the start of the interview (`caseStore.ts`), not re-shown on every visit.

- [x] **AA-37** Interview action-check step per the founder's 12 Sep direction. **DONE 12 Sep 2026** (exact commit not recoverable — same gap as AA-36). — **Owner:** AI assistant (build), Founder (the direction, already given). · **Cost:** $0. · **Deadline:** met. · **Blocks:** nothing — backfill of shipped work. — **Corrected 24 Sep 2026:** the interview step itself was retired on 22 Sep (`d9cb847`); its done / will do / cannot obtain choice lives on the workspace since 23 Sep (`0bbff36`, gap A-03). The tick describes that version.

### AM-25 — Header simplified to Decode · Dashboard · Vault; Case and Billing removed as nav slots (founder direction, 12 Sep 2026, backfilled 21 Sep 2026)

**Provenance:** same gap as AM-23/24 — cited in `src/components/AppHeader.tsx`, `src/components/ProfileMenu.tsx`, and `src/lib/useSignOut.ts` since 12 Sep 2026, never written up. This **reverses part of AM-21**, which had specified a five-slot header (Decode · Case · Dashboard · Vault · Pricing|Billing) as a locked access-ladder requirement — the reversal itself was never recorded as a decision anywhere until this backfill, which is exactly the kind of silent supersession `DECISION-PRINCIPLES.md` principle #2 exists to catch.

**What changed, per the code's own comment:** "Case" is intake, not a destination in its own right — Dashboard's "Start"/"Continue" button is the real entry point, so Case no longer gets a nav slot. Billing moves into the profile menu for a signed-in seller; there is no profile menu to hide Pricing behind when signed out, so Pricing stays a plain nav item only in that state. `useSignOut.ts` was extracted so `ProfileMenu` and any future entry point trigger identical sign-out behavior without nesting a button inside a button.

**Correction to AM-21's text:** AM-21's "(2) Header — five slots in both states" is superseded by this amendment for the header only; AM-21's other four points (tier ladder, storage, continuity, pricing/AI gating) are untouched.

- [x] **AA-38** Header simplification per the founder's 12 Sep direction. **DONE 12 Sep 2026** (exact commit not recoverable — same gap as AA-36/37). — **Owner:** AI assistant (build), Founder (the direction, already given). · **Cost:** $0. · **Deadline:** met. · **Blocks:** nothing — backfill of shipped work.

**A note on AA-36–38 versus the 19 Sep direction doc:** `docs/handoffs/2026-09-19-case-os-v2-direction.md` §6 had already proposed AA-36–39 for AM-26's four K0–K3 phases. Those phases have not been ratified — the founder's own second-opinion research pass was paused mid-way (`docs/handoffs/2026-09-19-case-os-v2-second-opinion-RESUME.md`) instead of returning a ratification, so AM-26 is deliberately **not** written into this file. Assigning AA-36–38 to this backfill instead means AM-26, if and when the founder ratifies it, will use AA-39 onward — a one-number shift from the direction doc's original guess, not a conflict, and worth a line in whatever prompt eventually executes it.

**Ratified 22 Sep 2026.** The founder gave the go-ahead in chat on 22 Sep 2026; AM-26 is written below and does use AA-39 onward, exactly as this note anticipated.

### AM-26 — Case OS v2: the decoder decides, the product reads the seller's documents, and the system speaks first (founder-ratified 22 Sep 2026)

**Provenance:** founder direction in chat, 22 Sep 2026, ratifying the direction drafted in `docs/handoffs/2026-09-19-case-os-v2-direction.md` §8 and reordering it against the 21 Sep commercial review. Full reasoning: `docs/handoffs/2026-09-22-case-os-v2-ratified-direction.md`; `docs/DECISIONS.md` entry of the same date.

**What the founder decided, in their own framing:** _"something is sold only when it is capable to be sold to relevant audience, and for that we need to work on our tool in most means."_ Product capability comes before price and before channel. The 21 Sep commercial review's conclusions on acquisition arithmetic are not overturned — they are **deferred**, because the review's own next action (ten unpaid seller conversations) turned out to rest on an assumption that is false: **the founder has no direct access to sellers or appeal writers.** Recruiting appeal writers is the intended route to both, and that recruitment needs a product strong enough to impress a professional first. Price stays at $249 and is explicitly parked for later discussion.
...
3. **The product reads the seller's uploaded documents.** Founder: _"we are not selling the vault, we are selling solution... to help them we need to read their files, we need to get help via API AI, wherever needed."_ This is a deliberate, founder-authorized narrowing of **D1/D6's "local-first"** — and only of local-first. See the D6 note below, which matters because the change is far smaller than it sounds.
...
- **"Read-only" is UNCHANGED and is not reopened.** It means AppealDeck never touches, automates, or acts on the seller's Amazon account. That is what keeps the product on the right side of the 4 Mar 2026 Agent Policy, and it stands exactly as written.
- **"Local-first" is what narrows** — and it had already narrowed without being recorded. Notice text has always been sent to the backend and on to Gemini (`/api/decode`, `/api/compose`, `/api/extract-field`, `/api/analyze-reply`); `legal/privacy.md` §2 already discloses this and the decode page's own badge already renders `processing="server"`. The genuinely local-only data today is **uploaded files**, and there is no file-upload endpoint in the repo at all. So this decision adds a capability that was never built rather than dismantling one that exists.
- **Everything else in D6 is untouched:** no "guarantee", honest-expectations before purchase, severity gating, win rates only from opt-in outcome data.
- **Recommended split, founder's call, does not block the build (AA-41):** business documents (invoices, authorization letters, sales reports, listing screenshots) are read server-side with AI, which is where reading creates the value the founder described; identity documents (passports, national ID cards, driving licences, bank statements) are checked in-browser for legibility, date validity and framing, without the image leaving the device. Rationale is liability, not ethics — the founder is personally liable as an individual, and a breach of a thousand invoices and a breach of a thousand identity documents are not the same event. The founder may direct full server-side processing instead; the architecture supports either.

**Build order (from the 19 Sep direction doc §5, unchanged):** K0 kernel → K1 one journey + the clock → K2 sensors → K3 pack and tracks.

- [x] **AA-39** **K0 — the kernel.** Response-type determination and entity extraction (ASINs, case/order IDs, dates, requested records with spans back to the source text); taxonomy v2 adding VERIFICATION, PERFORMANCE_METRIC, PRODUCT_SAFETY and RESTRICTED_PRODUCT; `routeWorkspace()` corrected so no notice family is left with no home. — **Owner:** AI assistant. · **Cost:** $0. · **Blocks:** AA-40, AA-41, AA-42.
- [x] **AA-40** **K1 — one journey, and the clock speaks first.** A real reminder delivery path (browser notification; email for signed-in sellers via the already-built Resend integration), the "since you were here" brief, and the waiting-on-a-third-party state. Journey consolidation is prepared but **gated on the founder's decision under point 5** — this item does not retire the classic interview. — **Owner:** AI assistant (build), Founder (the keep/retire call). · **Cost:** $0 build; needs `RESEND_API_KEY` for the email half. — **Corrected 24 Sep 2026:** the email half shipped; the browser-notification half is **struck**, not built (gap B-21: a notification that fires only while the tab is open does not speak first, and doing it properly needs a service worker and a push service). The classic interview was retired on 22 Sep, which answered point 5.
- [x] **AA-41** **K2 — sensors: the product reads the seller's documents.** Document reading per point 3 and the split above, each requirement resolving to present / missing / unclear / conflicting — never to the word "authentic", which no one outside Amazon can assert. Facts ledger with provenance; duplicate-submission guard before an identical resubmission. — **Owner:** AI assistant (build), Founder (the identity-document split, if they want it changed). · **Cost:** Gemini paid-tier usage, D9 ceiling and breaker apply.
- [x] **AA-42** **K3 — evidence pack and the remaining tracks.** Evidence pack export with a manifest; Seller Challenge token counter; verification-track preparation checklists. — **Owner:** AI assistant. · **Cost:** $0.
- [x] **AA-43** **Disclosure parity — ships in the same commit as AA-41, never after.** `legal/privacy.md` §1 currently states "We cannot read your case vault", which becomes false the moment AA-41 lands; `src/components/LocalFirstBadge.tsx` and the "Encrypted on your device" label in `src/content/marketing.ts` must match the real architecture. D6's honesty requirement makes this a blocking dependency of AA-41, not a follow-up. — **Owner:** AI assistant. · **Cost:** $0. · **Blocks:** AA-41 may not ship without it.
...
### AM-28 — The browser extension is dropped from the plan (founder decision, 24 Sep 2026)
...
notice into it exactly as they paste it into `/decode`. What it would add is a second codebase to
keep in step, a Chrome Web Store review, a second licence and kill-switch surface, and — the
decisive cost — a seller asking "does this touch my Amazon account?" of the one product whose
cleanest answer is that it never does. AM-26 builds the product to impress an appeal writer, and
"there is nothing to install" is part of that answer.

**Consequences.** Gap B-27 is closed as struck. The extension rows in `01-BUILD-SEQUENCE.md`
(B-23…B-27, M-6), the TRA-01/02/06/07/09/10 risk controls, AA-17 (kill-switch drill) and AA-18
(crxjs pin) describe work that will not happen; they stay in their files as history, and this
entry is authoritative over them, by the same convention as AM-27. Reversible: a future founder
decision can revive it, and would start from `01-BUILD-SEQUENCE.md` as written.

**No AA item.** There is nothing to build.

### AM-29 — More than one case per seller (founder direction 14 Sep 2026, backfilled 24 Sep 2026)

**Bookkeeping only.** The founder asked in chat on 14 Sep for AppealDeck to be "a true functional
Operating System for sellers", and the largest gap that surfaced was a single hardcoded case.
`09-MULTI-CASE-ARCHITECTURE-SPEC.md` planned the move and it was built: a case index in the vault,
an active-case pointer, the dashboard's case list and switcher, a "New case" action, evidence
listed per case, and — on 24 Sep — deleting a case with its files and its reminder. It was the only
multi-case work in the repository with no amendment record (gap C-08), so the next session could
...
- [ ] All 43 action items AA-01…AA-43 checked, or descoped with a reason logged in `docs/DECISIONS.md` (AA-32 is the visual refresh v3, appended by its Task V0; AA-33/AA-34 are AM-21, appended 10 Sep 2026; AA-35 is AM-22, appended 11 Sep 2026; AA-36–38 are AM-23/24/25, backfilled 21 Sep 2026 for already-shipped 12 Sep 2026 work — see the note above them; AA-39–43 are AM-26, ratified 22 Sep 2026, all DONE 22 Sep 2026 — see the 22 Sep status line below).
  - **22 Sep 2026 status (AM-26 ratified and built the same day):** AA-39 `0d65405` · AA-40 `dc9fbad` · AA-41+AA-43 `2627cc9` (one commit, as AA-43 requires) · AA-42 `dacaebf` · the facts ledger carry-forward in the commit following it. All five DONE and gate-verified; full detail and the discovered-defect list in `docs/handoffs/2026-09-22-case-os-v2-build.md`.
    - **Two things inside these items were deliberately NOT built, and are descoped rather than pending.** (1) The **Seller Challenge token counter** named in AA-42: Amazon never discloses how many challenges a seller has used, so any counter is self-reported, and presenting "you have 2 left" as authoritative is the same invented certainty as the approval-odds feature rejected on 19 Sep 2026. The rules themselves are already surfaced honestly in the deadline label. (2) **Browser push notifications** implied by AA-40's "browser notification": delivering one while the tab is closed needs a service worker, a push service and VAPID keys, and a notification that only fires when the page is already open does not "speak first" in any meaningful sense — email does that job honestly. Both reopen only on a founder decision.
    - **Founder calls, now resolved or still open:** the identity-document split (AA-41) was **implemented under the recommended split** — business documents server-side, identity and financial documents in-browser and never uploaded — because the founder said to proceed without ruling on it; reversible via `BROWSER_ONLY_EVIDENCE_KINDS`. The classic-interview keep/retire decision (AA-40) is **still open**; nothing in this pass retired it.
    - **External blockers unchanged:** migrations `0008` and `0011` need a manual Supabase apply, and `RESEND_API_KEY` is required before any reminder email sends. Both features are wired and visibly inert until then.
  - **11 Sep 2026 status (full-repo audit + execution pass — current):** DONE and verified in code: AA-01, AA-05, AA-08, AA-14, AA-15, AA-16, AA-19, AA-22, AA-24, AA-26, AA-27, AA-28, AA-29, AA-30, AA-32, AA-33. AM-16/AM-17 ratified this session (AA-19's founder half closed; see `docs/DECISIONS.md`). AA-21's AI-owned half (the EF-5 opt-in outcome schema) built this session (`src/core/outcomeModel.ts`). AA-31 (composer critic rules) built this session (`src/core/composer.ts`). Descoped with reason logged: the AA-10 working-spec-copy document (`docs/BUILD_PLAN.md`) — the engineering it would describe is already built, a second document adds no value (see the note on AA-10 above and `docs/DECISIONS.md`). Still genuinely open, no code substitutes for these: AA-02, AA-03, AA-04, AA-06, AA-07, AA-09, AA-11, AA-12, AA-13 (founder/infra tasks), AA-17 (extension kill switch — extension not started), AA-18 (the crxjs dependency pin itself — extension not started; the WXT-fallback decision is recorded), AA-20, AA-25, AA-34 (founder/consultant gates), AA-35 (visual overhaul v4 — V0 paperwork done, V1–V8 not started, two founder-gated decisions pending).
  - **Numbering correction (11 Sep 2026):** an earlier status line here swapped AA-15 and AA-18 — **AA-15 is the cost-ceiling/circuit-breaker item** (DONE, see above); **AA-18 is the crxjs-pin/WXT-fallback item** (still open, extension work hasn't started).
  - 10 Sep 2026: AM-20 ratified; AA-32 OPEN (visual v3) → **DONE 11 Sep 2026**, see above. — **9 Sep 2026:** AM-19 ratified by the founder; AA-30 (UI/UX polish pass) OPEN — prompt `docs/handoffs/2026-09-09-uiux-polish-prompt.md`; AA-31 rides M-4. — **10 Sep 2026:** AA-30 DONE via the fix pass (`cc1fe95`…`12b6a89`); founder screenshot sign-off pending; AA-31 still rides M-4 → **DONE 11 Sep 2026**, see above. — **2 Sep 2026 status:** AA-19, AA-22, AA-23, AA-24 DONE; AA-20, AA-21, AA-25 PENDING (founder/consultant gates). AA-15 (cost-ceiling breaker) and AA-18 (crxjs pin / WXT fallback rule) — AA-15 DONE, AA-18 still PENDING extension work (M-7/M-8, not M-W). — **5 Sep 2026:** AA-26 (Wave A) ✅ committed `67979dc`; AA-27 (Wave B) ✅ committed `e966c19`; AA-28 (Wave C) IN PROGRESS → **DONE 11 Sep 2026**; AA-29 (copy audit) — marketing+legal complete, app/auth strings move in Wave C → **DONE 11 Sep 2026**.
- [x] ~~The working spec copy (`V:\AppealDeck\docs\BUILD_PLAN.md`, per v1.0 §5) carries every amendment inline or by explicit pointer to this file~~ — **descoped 11 Sep 2026** (see AA-10 note above): the engineering this document was meant to describe is already built (`src/lib/llm/gemini.ts`); this file plus `docs/DECISIONS.md` already carry every amendment, so a separate working-spec copy adds no value. `reference/APPEALDECK_BUILD_PLAN_v1.0.md` itself remains UNMODIFIED as the historical record (repo paths inside it are original-authored text and are not corrected for the same reason).
