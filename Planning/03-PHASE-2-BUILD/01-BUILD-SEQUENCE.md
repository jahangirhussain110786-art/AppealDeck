...
| **Web revenue surface** | 4–5 | Free notice decoder + paid $249 composer live on the AppealDeck domain. Cloud Gemini Flash **paid tier** does the drafting; MoR (Paddle) checkout; license keys self-issued via Supabase. | First revenue with no store gate; catches mobile panic-searchers; revenue bridge if CWS review stretches weeks. [source: The Second Opinion.md] |
| **MV3 extension** | 4–8 | Panel decode → encrypted case vault → deadline alarms. **Paste-mode is the only architecture (AM-27, 22 Sep 2026).** ~~DOM-harvest~~ and ~~the POA-textarea injector~~ are removed from scope: Amazon's Agent Policy, in force 4 Mar 2026, prohibits Seller Central scraping and browser automation. | Paste-mode keeps full functionality with zero page access — which is now the only compliant option, and the strongest thing the product can say to an appeal writer choosing tools. |
...
3. ~~DOM-harvest code is not merged until the founder has retrieved and read the full BSA §19 / Agent Policy text.~~ **Moot 22 Sep 2026 (AM-27): there is no DOM-harvest code to merge.** The extension de-scoped to paste-only, which this rule always named as the outcome if the read went badly. The product works completely, as it said it would.
4. ~~The injector (writing the finished POA into Amazon's appeal textarea) is built last, behind a feature flag.~~ **Cancelled 22 Sep 2026 (AM-27, kill-criterion K2).** Writing into Seller Central is browser automation, which the Agent Policy prohibits. The standing rule survives it and is now absolute: never any automation or auto-submit, ever.
...
- [ ] **B-08** Retrieve the full BSA §19 / Agent Policy text (behind seller login) — via the founder's own fresh Amazon Individual seller account (~1–2 weeks, free) or a design partner's read-only secondary-user invite with written consent; deliver as PDF/text into `../07-REFERENCE/`. — **Owner:** Founder · **Cost:** $0 · **Blocks:** ~~any DOM-harvest code merge, injector decision, M-6 scope~~ — **nothing, as of 22 Sep 2026 (AM-27).** Both features are out of scope, so this no longer gates the extension. Still worth doing: it is the one place `docs/handoffs/2026-09-22-legal-boundaries-research.md` rests on reporting rather than the source text.
- [ ] **B-09** Provision hosting per AM-09 (settled default): **Cloudflare Pages (free)** hosts the static web decoder site; **Supabase Edge Functions** host ALL API work (MoR webhooks, LLM proxy, license verification). Supabase free tier at start → Pro $25/mo from first sustained sales. Vercel Hobby is contractually non-commercial — do not launch revenue on it; Vercel Pro ($20/mo) is NOT provisioned by default — it is a fallback only if porting the donor Express backend proves necessary at M-5, decided then by the AI assistant + Founder and logged (⚠ FOUNDER-DECISION only if it adds spend). — **Owner:** Founder (setup), AI assistant (deploy config) · **Cost:** $0–25/mo · **Deadline:** Week 1–2 · **Blocks:** M-W. **Corrected 24 Sep 2026 (gap D-02):** superseded by the 4 Sep single-host decision. The app deploys on **Vercel Pro** as one Next.js origin; there are no Supabase Edge Functions. Follow `docs/DEPLOYMENT.md`.
...
- [x] **B-15** ~~Read the retrieved BSA §19 text and rule on DOM-harvest and the injector.~~ **DONE 22 Sep 2026 — the ruling is de-scope both** (AM-27; `../00-DECISION/02-DECISION-LOG.md` §3 row 15). The founder made the call in chat on the published Agent Policy rather than a retrieved text, which is why B-08 stays open as a research item rather than a blocker. Extension host permissions reduce to none for Amazon domains. — **Owner:** Founder (decision, made), AI assistant (analysis, `docs/handoffs/2026-09-22-legal-boundaries-research.md` §4).
...
- [ ] **B-22** ⚠ FOUNDER-DECISION — price experiment: $99 vs $149 vs $199 across the first ~20 sales is a legitimate test (brief, correction 2). Decide whether to run it or hold the $199 anchor; if run, decide the split before sale 1 so the data is clean. — **Owner:** Founder · **Cost:** $0 (forgone revenue risk) · **Deadline:** before first public sale · **Blocks:** pricing copy, MoR product setup. **Superseded 21 Sep 2026 (commercial reset; gap D-03): the price is $249 flat, worldwide, and no price test is planned.**

### Weeks 4–8 — the MV3 extension (M-5, M-6)

- [ ] **B-23** Wrap the core in the extension: panel decode chip (**paste-mode only**), encrypted vault (M6 full), deadline alarms + notifications (M7 alarms on top of the core date math). ~~DOM-harvest merges ONLY if B-15 ruled it in~~ — B-15 ruled it **out** (AM-27); there is no DOM-harvest to merge and the extension requests no host permissions on Amazon domains. — **Owner:** AI assistant · **Cost:** $0 · **Deadline:** Weeks 4–6 · **Blocks:** M-6.
...
| Deadline kind | Rule |
|---|---|
| `appeal_window` | `noticeReceivedAt + statedDays` parsed from the notice text; ambiguous → shorter candidate + "verify in your notice" flag; missing → 90 (display as assumption). **Ratified divergence, 24 Sep 2026 (gap D-07):** the code does better than this row and the row is superseded. A missing window returns no date and tells the seller to confirm it in Account Health (`deadlinesModel.ts`); a stated length with no date on the notice runs "from the day you received it"; and the seller can now enter the date Account Health shows, labelled as theirs. An assumed 90 days would show a countdown Amazon never set. |
...
- [x] ~~The BSA §19 read happened before any DOM-harvest code merged~~ — **satisfied differently, 22 Sep 2026:** no DOM-harvest code exists or will, and the ruling is recorded (B-15, AM-27, `../00-DECISION/02-DECISION-LOG.md` §3 row 15).
