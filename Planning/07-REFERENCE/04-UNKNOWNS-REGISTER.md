...
| Can hosting be $0 through launch? | **Superseded 4 Sep 2026 — this row's original answer was wrong for the architecture actually shipped.** The founder decided single-host Vercel (marketing + auth + app on one origin, see `CLAUDE.md` §4 and `docs/DEPLOYMENT.md`), and the app deliberately runs on **Vercel's free Hobby tier** through the first cohort of customers, with an explicit upgrade trigger once revenue justifies Pro (see `AGENTS.md`). Hobby's terms restrict *commercial resale of the platform itself*, not running a paid product on it during early validation — this is common practice and the founder's own informed choice, not an oversight. | Launch stack **as actually built:** Vercel Hobby (free) + the project's own fresh Supabase project (free tier). Upgrade to Vercel Pro is a founder decision once paid usage justifies the cost — not a Day-1 requirement. The Cloudflare Pages / Hetzner alternative below was never built and is not the current plan. |
