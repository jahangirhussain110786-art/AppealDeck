...
**Terms:** M-1…M-8 = the build plan's eight weekly milestones (M-1 scaffold → M-8 first paid customer). CWS = Chrome Web Store. MoR = Merchant of Record (Paddle — the reseller that handles customer-country VAT and acts as seller of record). BSA §19 = Amazon's Business Solutions Agreement "Agent Policy" (effective 4 Mar 2026), which restricts automated access to Seller Central. DOM-harvest = the extension feature that reads the deactivation notice off the Seller Central page. POA = Plan of Action (the appeal document). UPL = unauthorized practice of law. E&O = errors & omissions insurance. GDPR Art. 30 = the EU-required record of data-processing activities; DPA = data processing agreement with each subprocessor. Dexie = the IndexedDB wrapper used for the encrypted local vault. VAMP = Visa Acquirer Monitoring Program (chargeback-ratio regime).
...
- [ ] **2. Collaborator & Contractor Policy adopted.** Pass: the policy (roles, IP assignment, credential handling, non-solicit) is written and adopted, so any future contractor, VA, or expert is bound by it **before receiving any credential or IP access** — solo founder, so no partnership agreement exists and adoption costs nothing (see `../01-PHASE-0-BLOCKERS/02-COLLABORATOR-POLICY.md`). **Owner:** Founder · **Cost:** $0 · **Deadline:** Day 1 · **Blocks:** any future credential sharing, talent recruitment.
- [ ] **3. Paddle application submitted.** Pass: application in progress behind the live site + legal pages — do not wait for approval to proceed. **Owner:** Founder · **Cost:** $0 · **Deadline:** Week 1 · **Blocks:** M-5 payments.
- [ ] **4. Domain + legal pages live.** Pass: domain registered; AI-drafted, founder-reviewed privacy policy + terms published (covering disclaimers, refund policy per D8, data flows). **Owner:** Founder (publish) + AI assistant (draft) · **Cost:** $10–30/yr · **Deadline:** Week 1 · **Blocks:** Paddle application, CWS listing, web decoder.
- [ ] **5. Polar warm-fallback account created.** Pass: Polar account open on the free tier, with the Pakistan payout path verified at signup (Polar pays out to Pakistan via Stripe Connect cross-border — confirm it works for this account before relying on the rail); kept warm, not integrated. Dodo Payments (MoR, 4% + 40¢) is plan-C: application kept ready, no account opened unless triggered. **Owner:** Founder · **Cost:** $0 (free tier) · **Deadline:** Week 1 · **Blocks:** payment fallback path (K1, K13).
- [ ] **6. Paddle sandbox account created.** Pass: sandbox account created and API credentials handed to the AI assistant for integration work. **Owner:** Founder · **Cost:** $0 · **Deadline:** Week 1 · **Blocks:** webhook integration, test-mode E2E.
- [ ] **7. CWS developer account active + trader contact details published.** Pass: $5 fee paid; verified contact details (name/business name, address, email, phone) submitted and visible on the listing. **Owner:** Founder · **Cost:** $5 · **Deadline:** Week 1 · **Blocks:** M-7 store submission (verification can take weeks — start early).
...
- [x] **9. Repo scaffolded.** Pass: `V:\AppealDeck1\` code workspace created from clean/cleared sources only (forbidden-source ban per `02-DECISION-LOG.md` Round 0); CI green on the empty app. **Owner:** AI assistant · **Cost:** $0 · **Deadline:** Week 1 · **Blocks:** M-1 acceptance.
...
- [ ] **11. Payout account verified.** Pass: personal Payoneer or Wise personal account open and verified in the founder's own name (Jhangir Hussain). **Owner:** Founder · **Cost:** $0 setup · **Deadline:** Week 1 · **Blocks:** MoR payout receipt, contractor payments.
...
**Gate 1 kill criterion — K1:** If Paddle rejects outright → **pause the build** until a payment rail exists. Pre-agreed response: activate Polar, the warm fallback (check 5 — Pakistan payout runs via Stripe Connect cross-border, verified at signup); if Polar fails too, trigger plan-C and apply to Dodo Payments (MoR, 4% + 40¢); if none work, this is NO-GO trigger 1 in `01-VERDICT.md` §(f).
...
- [ ] **15. Manual QA clean on live Seller Central.** Pass: smoke test on a real account (active-access ladder per `02-DECISION-LOG.md` §3 row 6) passes; no console errors. **Owner:** Founder · **Cost:** $0 · **Deadline:** Week 6–7 · **Blocks:** M-6 acceptance.
...
- [ ] **17. ⛔ HARD ITEM — BSA §19 / Agent Policy full text read, for any DOM-reading feature.** Pass: the founder has retrieved the full policy text from behind a seller login — via the founder's own fresh Amazon Individual seller account (free, ~1–2 weeks to open) or a design partner's read-only secondary-user invite with written consent; it has been read and assessed against the DOM-harvest and injector mechanisms; the assessment is written into `02-DECISION-LOG.md`. If the read is adverse: DOM-harvest is de-scoped to paste-only (K8) and the injector is cancelled — the product ships regardless, on the paste-mode spine. A paste-only build with no DOM-reading features may pass this check by confirming that scope in writing. **Owner:** Founder (retrieve + decide) + AI assistant (assess) · **Cost:** $0–500 (optional counsel review) · **Deadline:** text retrieved before end of Week 2; ruling recorded in `02-DECISION-LOG.md` by end of Week 3; always before any DOM-harvest code merges (Weeks 4–6); verified hard at Gate 2 · **Blocks:** DOM-harvest, injector, CWS listing risk.
...
- [ ] **19. Consumer withdrawal-right compliance implemented.** Pass: checkout collects explicit prior consent to immediate digital delivery + sends permanent-form confirmation, per applicable consumer-protection law (EU Directive 2011/83/EU for EU buyers, UK consumer law for UK buyers, etc.). Seller location is Pakistan; Paddle is the MoR. **Owner:** AI assistant (build) + Founder (verify) · **Cost:** included · **Deadline:** before M-5, verified at Gate 2 · **Blocks:** market legality, MoR compliance.
...
|---|---|---|---|
| K6 | Chargeback ratio climbing | >0.4% (review level) or >0.5% (internal alarm) → escalating to 0.75% (processor monitoring) / 1.5% (Visa VAMP freeze) | At 0.5%: pause paid acquisition, refund faster and more proactively, audit the honest-expectations flow. At any processor-program entry: daily monitoring, alert the MoR proactively, refund-first policy on every complaint. An MoR termination is existential. |
| K7 | Support overload / burnout | >20 tickets/day for >3 consecutive days, or founder response time >24h | Pause marketing spend; deploy canned responses; switch support to an honest auto-reply and answer in a founder morning batch (a future vetted VA, restricted to approved canned responses per the collaborator policy, may take this over); hard work-hour boundaries enforced. |
...
| K10 | Mass-suspension demand spike | Decoder traffic >3x baseline in 24h, or community reports ~10x normal suspension volume | Cloud circuit breaker holds the spend cap; graceful degradation to rules-only decode; canned responses out; founder does NOT answer individually — an honest auto-reply plus batched canned responses handle triage, with design partners helping in community threads. |
...
| K13 | Payment-rail deterioration | MoR requests information, flags the account, or delays payouts | Respond same day; warm fallback rail (Polar — check 5) readied for activation within 48h, with Dodo Payments as plan-C behind it; never let a single rail be a single point of failure. |
...
- [ ] **41. Hold the weekly decision review — every week, same slot, 30 minutes, no skipping.** **Owner:** Founder · **Cost:** $0 · **Deadline:** weekly from Week 1 · **Blocks:** kill-criteria enforcement, decision-log integrity.
