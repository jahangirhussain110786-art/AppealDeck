...
- **Pending commits (not yet pushed; will be one commit before session end):**
  ...
  - **A11y fix — Alert color contrast on `/decode`.** `src/components/ui/alert.tsx`: changed all four alert variants (`info`, `warning`, `destructive`, `success`) from `text-*-foreground` (white in light mode) to `text-foreground`, and the same for `alertTitles`. Root cause: `bg-info/10` produces a light-blue tint (`#e8edfa`) in light mode while `text-info-foreground` is white → 1.17:1 contrast (fails WCAG 2 AA 4.5:1). White text was only correct for full-opacity badge backgrounds, not for `/10` tint backgrounds. Badges in `badge.tsx` keep their `*-foreground` colors (full-opacity `bg-*-*` backgrounds pass contrast). Validation: typecheck ✅, lint ✅, lint:copy ✅, format:check ✅, Playwright 44/44 (was 43/44), vitest 287/287.
