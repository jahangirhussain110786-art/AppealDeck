...
- **Last updated:** 4 Sep 2026 (morning — full auth shipped + dev user rotated + ready for end-to-end testing)
...
- **Founder action items (NEW from this auth session, status):**
  - ⏸ **Supabase dashboard → Authentication → URL Configuration → Redirect URLs:** add `http://localhost:3000/auth/callback` and `https://app.appealdeck.com/auth/callback` (for prod). Without these, Google sign-in fails with a redirect-URL-mismatch error. Email magic links work without dashboard changes because they use the `emailRedirectTo` option (which is now `http://localhost:3000/auth/callback` thanks to `NEXT_PUBLIC_APP_URL`).
  - ⏸ **Supabase dashboard → Authentication → Providers → Google:** enable, paste the Google OAuth client ID + secret. Skipped for now — Google sign-in button is wired in the UI but the provider is disabled in Supabase; the user gets a clean error from the OAuth flow.
  - ⏸ **Supabase dashboard → Authentication → Email → Confirm email:** currently enabled (the default). If you want the dev user (and future production users) to sign in without clicking a confirmation link, set this off — but the trade-off is no email-verification step. The admin-created dev user bypasses this with `email_confirm: true`.
  - ✅ `NEXT_PUBLIC_APP_URL=http://localhost:3000` + `NEXT_PUBLIC_SITE_URL=http://localhost:3000` set in `.env.local` so email/Google redirect to localhost in dev, not to `app.appealdeck.com` (which doesn't resolve in dev).
  - ✅ Dev user `dev@appealdeck.com` created and confirmed.
- **Validation:** typecheck/lint/build all green; **230/230 Vitest across 21 files**; 23 routes + middleware; dev server serves 200 on `/`, `/decode`, `/pricing`, `/login`, `/signup`, `/forgot-password`, `/reset-password`, `/case` (redirects to `/login` when unauthed), `/compose`, `/billing`, `/vault`. Playwright visually confirmed: login shows Google+forgot+create-account, signup shows create-account form, forgot shows the reset form.
