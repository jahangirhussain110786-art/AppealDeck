...
- **Last updated:** 3 Sep 2026 (evening, AM-11 device-activation pass + skeptic cleanups)
- **AM-11 device-activation pass — clean (3 Sep 2026, after skeptic pass):** ship + the four real issues found by the inline skeptic pass are now fixed. **Shipped:** `supabase/migrations/0006_device_activations.sql`, `src/lib/devices.ts` (`DEVICE_CAP=5`, `fingerprintFromRequest`, `recordActivation` with race-recovery via 23505 detection, `listDevices`, `revokeDevice`), `src/app/api/devices/route.ts` (GET + DELETE — POST deleted as dead code, was YAGNI), `src/components/DeviceManager.tsx`, wiring into `src/app/api/compose/route.ts:46-66` + `src/components/ComposeView.tsx` (structured `errorKind` state — replaced fragile regex on error string). 15 device tests in `src/lib/__tests__/devices.test.ts` (was 13, +2: fingerprint-doesn't-require-email, revoke-then-reactivate-un-revokes). **Real bugs found by skeptic + now fixed:** (1) revoke-then-reactivate previously returned silent 200 with no device row because the `UNIQUE(license_id, fingerprint)` constraint blocked the re-insert and the error branch swallowed it; now un-revokes in place. (2) Race at the 5/5 boundary could allow 6th device if two requests at 4/5 both saw `count=4` and both inserted — now detects 23505 (unique_violation) and demotes to over-cap with a `console.warn`. (3) `email` was in the fingerprint input as PII (hashed but redundant with `user_id`); removed. (4) Dead `POST /api/devices` handler deleted. (5) `useEffect` device-cap detection was a fragile `/device cap|device limit|revoke/i` regex on the error string — now a typed `errorKind: "device_cap" | "generic"`. **Also tightened:** the AM-06 ethics guard test (`src/core/index.test.ts:18-27`) was using `/\bguarantee\b/i` which silently lets "guaranteed" through (word-boundary doesn't trigger before "d"); now `/\bguarantee\b|\bguaranteed\b/i` and two core copy lines (guidance.ts, caseState.ts) rephrased. User-facing "guarantee" removed from `app/layout.tsx`, `app/terms/page.tsx`, `components/SiteFooter.tsx`, `app/pricing/page.tsx`, `app/decode/page.tsx`. **Totals now: 188/188 Vitest tests, 16 test files, 21 routes + middleware, typecheck/lint/format:check/build all green.** Cap deliberately starts at 5 (the plan's "3–5" upper bound) — tighten only on observed abuse per TRC-12. Founder action: apply `0006_device_activations.sql` to the live Supabase project.
- **Pending audit items (3 Sep 2026, not yet shipped — see the parallel `code-skeptic` audit on prior work):**
  - AM-12 circuit breaker + cloud cost ceiling + per-device IP rate limits + rules-only degradation — **not in code**. The plan's "rules-only fallback" is satisfied by accident because there's no LLM in the repo yet. AA-15 = ⛔ not done.
  - AM-14 / AA-17 remote-settings kill switch + drill — **not in code**. AA-17 = ⛔ not done.
  - AM-15 / AA-18 crxjs pin + WXT fallback — **not in code**. The extension itself doesn't exist; this is M-7/M-8 work. ⛔ N/A until then.
  - AM-07 LLM inversion (cloud paid tier is the primary quality path) — **de facto deferred** — no Gemini in repo. AGENTS.md should be honest; the production-readiness pass already ships without LLM, so M-W already works on rules-only. ⏸ deferred.
  ...
  - Playwright e2e has no auth setup — specs never log in, so changing `requireUser()` has no test signal. ⏸ next on the list after AM-12.
  - `/api/analyze-reply` is unauthenticated + rate-limit-less (abuse vector, but rules-only so cheap). ⏸ quick win, ~30 min.
  ...
  - Founder action: apply `0006_device_activations.sql` to live Supabase, then commit + push the 9+ staged changes.
- **Infra cleanups (3 Sep 2026):** `vitest.config.ts` created — Vitest now scopes to `src/**/*.{test,spec}.{ts,tsx}` and excludes `e2e/**` (the 2 Playwright spec files used to be picked up by Vitest and fail with a `Cannot find module '@playwright/test'` error). `.gitignore` now also ignores `.mcp-bridge-*.cmd` and `appealdeck.md` (stray workspace artifacts). Staged 10 uncommitted deletions of stale `.claude/skills/paddle-*/SKILL.md` (the real skills are in `.agents/skills/paddle-*/`). **Not done — your call:** commit + push to `origin/master` (9+ commits ahead, dirty tree now clean).
