...
- **Validation:** typecheck/lint/build all green; **233/233 Vitest across 21 files**; 23 routes + middleware; dev server serves 200 on `/`, `/decode`, `/pricing`, `/login`, `/signup`, `/forgot-password`, `/reset-password`, `/case` (redirects to `/login` when unauthed), `/compose`, `/billing`, `/vault`. Playwright end-to-end smoke test (4 Sep 2026, after license row + Google config + Gemini fix): signed in as `dev@appealdeck.com`; `/case` shows the full Guided Interview (7 enforcement-type chips, "How this works" section, no license gate); `/billing` shows "Appeal Pass — active" + AM-11 device panel; `/vault` shows the passphrase setup screen; "Suggest fields (AI)" fires `/api/extract-field` which now returns `ok:true` with real AI suggestions (`suggestedKind:INAUTHENTIC_DOCUMENTS, suggestedSeverity:high, suggestedTimelineSummary:...`); "Continue with Google" correctly redirects to `accounts.google.com` with the Supabase callback + state. Sign-out 404'd on `/app/login` (bug from `882ed39` round 2), fixed in pending commit.
