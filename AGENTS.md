...
- **UI aligned to global `frontend-specialist` spec (full alignment):** installed `framer-motion`, `lucide-react`, `clsx`, `tailwind-merge`, `class-variance-authority`, `next-themes`, `@radix-ui/react-slot`; added `cn()` (`src/lib/utils.ts`) + shadcn-style primitives (`src/components/ui/{button,card,input,textarea}.tsx`); `next-themes` light/dark with a system-preference toggle (`ThemeToggle`) and HSL CSS-var tokens in `tailwind.config.ts` + `globals.css` (brand emerald as `--primary`, soft shadows/depth instead of harsh borders); rebuilt every page/component on primitives + `cn()` + Lucide icons + subtle Framer micro-interactions. `frontend-specialist.md` is now the binding default UI standard for all future work. 22 Vitest tests green; `next build` green (13 routes).
