...
if (!name || !pkg) {
  console.error(
    'Usage: node mcp-bridge-up.mjs --name X --port N --package "@org/pkg" [--args a b c] [--env KEY1 KEY2 ...]',
  );
  process.exit(2);
}
...
if (existsSync(pidFile)) {
  ...
  if (pid && pid > 0) {
    try {
      process.kill(pid, 0);
      console.log(
        `mcp-bridge[${name}] already running (pid ${pid}) on http://127.0.0.1:${port}/mcp`,
      );
      process.exit(0);
    } catch {}
  }
...
const probe = () =>
  new Promise((resolveP) => {
    const req = http.request(
      { host: "127.0.0.1", port: Number(port), path: "/healthz", method: "GET", timeout: 1500 },
      (res) => {
        res.resume();
...
while (Date.now() < deadline) {
  ...
  if (r2.ok) {
    ...
    console.log(
      `mcp-bridge[${name}] ready${discoveredPid ? ` (pid ${discoveredPid})` : ""} on http://127.0.0.1:${port}/mcp`,
    );
