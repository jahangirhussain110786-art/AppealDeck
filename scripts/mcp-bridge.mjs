...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === PATH_MCP) {
    try {
      ...
      console.error(`[bridge:${NAME}] handleRequest err:`, e?.message ?? e);
      if (!res.headersSent) {
        res.writeHead(500);
        res.end();
      }
...
console.error(
  `[bridge:${NAME}] HTTP listening on http://127.0.0.1:${PORT}${PATH_MCP} (pid ${process.pid})`,
);
