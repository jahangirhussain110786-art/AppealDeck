...
const probe = () =>
  new Promise((resolveP) => {
    const req = http.request(
      { host: "127.0.0.1", port: Number(port), path: "/mcp", method: "GET", timeout: 1500 },
      (res) => {
        res.resume();
        resolveP({ ok: true });
      },
...
while (Date.now() < deadline) {
  ...
  if (r2.ok) {
    ...
    console.log(
      `google-drive MCP ready${discoveredPid ? ` (pid ${discoveredPid})` : ""} on http://127.0.0.1:${port}/mcp`,
    );
...
console.error(
  `google-drive MCP did not become ready within 60s.${lastRefused ? " (port refused — another process may be bound)" : ""} Check ${logFile}.`,
);
process.exit(1);
