...
| 8 | A payments path exists for a Finnish solo founder | **CONFIRMED — with a rail change** | Lemon Squeezy is sunsetting (do not build on it). Paddle: flat 5% + $0.50, deepest tax coverage. Polar.sh: free 5% + 50¢, Pro $20/mo 3.8% + 40¢ — cheapest at low volume. Both onboard Finnish sole traders; Merchant of Record (MoR) genuinely removes customer-country VAT. Constraint: MoR acceptable-use policies require automated software output — human-service tiers (Expert Review) are not MoR-eligible. |
| 9 | Company formation is fast and cheap | **CONFIRMED** | Finnish toiminimi (sole-trader registration) €75 via ytj.fi, ~3–5 business days (PRH 2026 price list). Not on the critical path. |
...
2. **BSA §19 / Agent Policy risk is higher than the early documents treated it.** Resolution: (a) web decoder + paste-mode is the **primary architecture and compliance spine** (zero page access, full functionality); (b) extension DOM-harvest (reading the notice off the page) is a convenience layer gated on reading the actual §19 text (Jhangir, before milestone M-3) — de-scoped to a paste-only extension if the read is bad; (c) the POA-textarea injector is the riskiest feature and ships **last or never**; (d) never any automation or auto-submit.
...
- [ ] **2. Blockers closed before build.** Leaked Supabase credentials rotated; written Jhangir partnership agreement signed **before** any credential sharing; toiminimi registered; Paddle **and** Polar applications submitted Week 1 behind a live site + legal pages. **Owner:** Founder (+ Jhangir for the agreement) · **Cost:** ~€285–2,105 (agreement €200–2,000 + toiminimi €75 + domain €10–30) · **Deadline:** Day 1–Week 1 · **Blocks:** everything. See `../01-PHASE-0-BLOCKERS/`.
...
1. **No payment rail:** Paddle rejects outright AND neither Polar nor Stripe direct is operational within 48h of that rejection (Gate 1 kill).
